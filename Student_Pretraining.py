...
parser = argparse.ArgumentParser(description='Student-PreTraining')
...
parser.add_argument('--learning_rate', type=float, default=0.0002,
                    help='Learning rate of the student model. Default: 0.0002')
...
parser.add_argument('--continuous_saving', default=False, action='store_true',
                    help='Save the student model after each epoch? Default: false')
...
def data_semantic_info():
    ...
    # For the BraTS dataset, the number of student modalities/sequences (i.e., those that are pre-contrast) is 3.
    num_mods_student = 3
    
    # Create semantic info.
    semantics = ((set_x, set_y, set_z), num_mods_student)
    return semantics
...
def get_dataloaders(list_ids, root_paths, semantics, transform):
    ...
    data_gen_train = data_generator_pretraining(list_ids[0], root_paths[0], "student", transform, True, semantics)
    ...
    data_gen_val = data_generator_pretraining(list_ids[1], root_paths[1], "student", None, False, semantics)
    ...
    if(args.testing_set):
        data_gen_test = data_generator_pretraining(list_ids[2], root_paths[2], "student", None, False, semantics)
...
def get_model(device):
    ...
    model = Model(num_mods=3, k=args.init_num_filters, p=args.dropout_prob, num_class=4).to(device)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if(validate_vm_list[-1] >= np.max(validate_vm_list)):
            torch.save(model.state_dict(), "Pre_Trained_Student_BEST_VAL_VM.pth")
        ...
        if(args.continuous_saving):
            torch.save(model.state_dict(),"Pre_Trained_Student_Model.pth")
            torch.save(optimizer.state_dict(), "Pre_Trained_Student_Optimizer.pth") 
            torch.save(scheduler.state_dict(), "Pre_Trained_Student_Scheduler.pth")
    ...
    original_dir = os.getcwd()
    os.chdir(args.save_state_dir)
    model.load_state_dict(torch.load("Pre_Trained_Student_BEST_VAL_VM.pth"))
    os.chdir(original_dir)
